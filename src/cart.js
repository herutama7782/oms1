    ...
    try {
        ...
        if (results.length > 0) {
            searchResultsEl.innerHTML = results.map(c => `
                <div class="p-3 hover:bg-gray-100 cursor-pointer" onclick="selectCustomer(${c.id}, '${c.name.replace(/'/g, "\\'")}')">
            ...
            searchResultsEl.innerHTML = '<p class="p-3 text-gray-500">Pelanggan tidak ditemukan.</p>';
            searchResultsEl.classList.remove('hidden');
        }
    } catch (error) {
        console.error('Error searching customers:', error);
        searchResultsEl.innerHTML = '<p class="p-3 text-red-500">Gagal mencari.</p>';
        searchResultsEl.classList.remove('hidden');
    }
}
